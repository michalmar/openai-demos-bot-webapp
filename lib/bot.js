    ...
    constructor() {
        ...
        this.onMessage((context, next) => __awaiter(this, void 0, void 0, function* () {
            ...
            if (attachments && attachments[0] && attachments[0].contentType === 'application/vnd.microsoft.teams.file.download.info') {
                ...
                yield writeFile(file.content.downloadUrl, config, filePath);
                const reply = botbuilder_1.MessageFactory.text(`<b>${file.name}</b> received and saved.`);
                ...
                console.log("Recieved TEXT: " + attachments[0].contentType);
                const file = attachments[0];
                const filePath = path.join(FILES_DIR, file.name);
                const config = {
                    responseType: 'stream'
                };
                yield writeFile(file.contentUrl, config, filePath);
                const fileContent = yield fs.promises.readFile(filePath, 'utf-8');
                yield context.sendActivity(botbuilder_1.MessageFactory.text("FileText: " + fileContent));
            }
            else if (attachments && attachments[0] && imageRegex.test(attachments[0].contentType)) {
                yield this.processInlineImage(context);
            }
            else {
                ...
                yield context.sendActivity(botbuilder_1.MessageFactory.text("Just text: " + context.activity.text));
            }
        ...
        function getFileFromRequestAndReadAsText(context) {
            return __awaiter(this, void 0, void 0, function* () {
                if (context.activity.attachments && context.activity.attachments.length > 0) {
                    ...
                    yield fs.promises.writeFile(filePath, attachment.contentUrl, 'base64');
                    const fileContent = yield fs.promises.readFile(filePath, 'utf-8');
                    return fileContent;
                }
                else {
